...
func NewMainApp(appVer AppVersion) *cli.Command {
	app := &cli.Command{}
	app.Name = "gitfx" // must be lower-cased because it appears in the "USAGE" section like "gitfx doctor [command [command options]]"
	app.Usage = "A painless self-hosted Git service"
	app.Description = `GitFX program contains "web" and other subcommands. If no subcommand is given, it starts the web server by default. Use "web" subcommand for more web server arguments, use other subcommands for other purposes.`
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			...
			Aliases:   []string{"w"},
			TakesFile: true,
			Usage:     "Set GitFX's working path (defaults to the GitFX's binary directory)",
		},
		&cli.StringFlag{
			Name:      "config",
			Aliases:   []string{"c"},
